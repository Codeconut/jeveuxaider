<template>
  <div class="profiles has-full-table">
    ...
    <div class="px-12 mb-12">
      <profiles-menu
        index="/dashboard/profiles/referents-regions"
      ></profiles-menu>
    </div>
    ...
    >
      ...
      <el-table-column label="Région" width="200">
        <template slot-scope="scope">
          <div class="text-gray-600">
            {{ scope.row.referent_region }}
          </div>
        </template>
      </el-table-column>
      <el-table-column label="# Structures" width="150" align="center">
        <template slot-scope="scope">
          <div class="text-gray-900">
            {{
              scope.row.referent_region_waiting_actions.structures
                | formatNumber
            }}
          </div>
      ...
      <el-table-column label="# Missions" width="150" align="center">
        <template slot-scope="scope">
          <div class="text-gray-900">
            {{
              scope.row.referent_region_waiting_actions.missions | formatNumber
            }}
          </div>
          <div class="font-light text-gray-600 text-xs">en attente</div>
        </template>
      ...
      <el-table-column label="# Actions" width="150" align="center">
        <template slot-scope="scope">
          <div class="text-gray-900">
            {{ scope.row.referent_region_waiting_actions.total | formatNumber }}
          </div>
          <div class="font-light text-gray-600 text-xs">en attente</div>
        </template>
      </el-table-column>
...
<script>
import { fetchProfiles, exportProfilesReferentsRegions } from '@/api/user'
...
export default {
  ...
  methods: {
    fetchRows() {
      return fetchProfiles(
        ...
        [
          ...
          'skills',
          'domaines',
          'referent_region_waiting_actions',
        ]
      )
    },
    ...
    onExport() {
      this.loading = true
      exportProfilesReferentsRegions({
        ...this.query,
        'filter[role]': 'referent_regional',
      })
        .then((response) => {
          this.loading = false
          fileDownload(response.data, 'referents-regions.csv')
        })
        .catch((error) => {
          Message({
